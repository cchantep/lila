...
final class Env(
  ...
  lazy val db = {
    ...
    val warmup = Duration("5s")

    registerDriverShutdownHook(connection, driver)

    parsedUri.db.fold[DefaultDB](sys error s"cannot resolve database from URI: $parsedUri") { dbn =>
      val db = Await.result(connection.database(dbn), warmup)
      loginfo(s"""ReactiveMongoApi successfully started with DB '$dbn'! Servers:\n\t\t${parsedUri.hosts.map { s => s"[${s._1}:${s._2}]" }.mkString("\n\t\t")}""")
      db
    }
  }
